...
const SVG_ID_ALIAS = {
  ...
  '角楼-3':    'jiaolou',
  '角楼-4':    'jiaolou',
  // SVG ID 与 BUILDINGS key 映射（新SVG中的ID）
  'wumen': 'wumen',
  'qinandian': 'qinandian',
  'wuyingdian': 'wuyingdian',
  'wenhuadian': 'wenhuadian',
  'jianting': 'jianting',
  'taihemen': 'taihemen',
  'kunninggong': 'kunninggong',
  'qianqingmen': 'qianqingmen',
  'qianqinggong': 'qianqinggong',
  'baohedian': 'baohedian',
  'changyinge': 'changyinge',
  'yangxingdian': 'yangxingdian',
  'xiliugong': 'xiliugong',
  'shenwumen': 'shenwumen',
  'shoukanggong': 'shoukanggong',
  'cininggong': 'cininggong',
  'dongliugong': 'dongliugong',
  'cininggonghuayuan': 'cininggonghuayuan',
  'taihedian': 'taihedian',
  'jiaolou': 'jiaolou',
  'yuhuayuan': 'yuhuayuan',
  'qianlonghuayuan': 'qianlonghuayuan',
  'huangjidian': 'huangjidian',
  'yangxindian': 'yangxindian',
  'jiulongbi': 'jiulongbi',
  'shufangzhai': 'shufangzhai',
  // SVG中拼写错误 → 正确ID
 ...
 * 优先级：别名 → 直接英文key → 按 name（英文名）匹配 → 按 name（中文名）匹配 → 回退 svgId
 */
function resolveBuildingId(svgId) {
  // 首先检查别名映射
  if (SVG_ID_ALIAS[svgId]) return SVG_ID_ALIAS[svgId]
  // 检查是否直接是 BUILDINGS 的 key（英文）
  if (BUILDINGS[svgId]) return svgId
  // 按 name 英文名匹配（data.js 中的 nameEn）
  const byNameEn = Object.values(BUILDINGS).find(b => b.nameEn === svgId)
  if (byNameEn) return byNameEn.id
  // 按 name 中文名匹配（data.js 中的 name）
